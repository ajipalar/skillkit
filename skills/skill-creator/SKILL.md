...
# Original copyright Anthropic, PBC. Licensed under Apache 2.0.
# Modified by Ajikarunia Palar, 2026 — reworded to be model-agnostic.
---
name: skill-creator
...
When creating a new skill from scratch, you can use the `init_skill.py` script from the Anthropic reference implementation (included in `scripts/`). Note that this script and the packaging workflow below are specific to Claude Code's skill system — adapt or omit them when targeting other platforms.
...
Once development of the skill is complete, it can be packaged into a distributable `.skill` file using the Anthropic reference implementation script. Note that the `.skill` format is specific to Claude Code — this step is optional and platform-dependent for other agent frameworks.
